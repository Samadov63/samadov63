...
public://Публичные свойства(методы)
	...
	bool operator>=(int age);//Перегрузка для сравнения возраста, теперь можно заместо использования метода Вотер, сразу задавать нужный возраст
	//bool Lastname(std::string surname);//сопостовление фамилии
	bool operator ==(std::string surname);
...
bool TMan::operator==(string surname)
{
...
int main()
{
	...
	for (int i = 0; i < N; i++)
	{
		if (ManArray[i] >= 18 && ManArray[i] == sur)
		{
			cout << ManArray[i];
			cout << endl;
			kol++;
