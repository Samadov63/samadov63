                        ...
                        PhoneERR:
                            try
                            {
                                ...
                                if(value.Contains("+"))
                                {
                                    plus = true;
                                    if (value[0] != '+')
                                        throw new Exception("не верно введён телефон (плюс стоит не первым символом)");
                                    if (value.Split('+').Length > 2)
                                        throw new Exception("не верно введён телефон (много знаков плюс");
                                ...
                                if (value.Length > 10)
                                {
                                   // throw new Exception("Введены не корректные данные");
                                    if (value[0] == '0')
                                    {
                                        throw new Exception("у мобильного первой цифрой не может быть ноль, если не стоит первым символом плюс");
                                    }

                                    record.Phone = value.Substring(0, value.Length - 10);
                                    if (!plus)
                                    {
                                        int i = int.Parse(record.Phone);
                                        record.Phone = (i - 1).ToString();
                                    }

                                    record.Phone = "+" + record.Phone;
                                    record.Phone = record.Phone + long.Parse(value.Substring(value.Length - 10)).ToString("(###)###-##-##");
                                ...
                                if(value.Length == 7)
                                {
                                    record.Phone = record.Phone + long.Parse(value.Substring(value.Length - 7)).ToString("###-##-##");
                                    //
                                }
                                else
                                {
